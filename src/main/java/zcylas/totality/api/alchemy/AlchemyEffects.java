...
public final class AlchemyEffects {
    ...
    public static final AlchemyEffect RESTORE_STAMINA = AlchemyEffect.register(
            ...
            .withConsumeEffect(entity -> {
                if (entity instanceof net.minecraft.server.level.ServerPlayer player) {
                    zcylas.totality.api.stamina.PlayerStaminaManager.addStamina(player, 5);
                    zcylas.totality.networking.stamina.StaminaServerTick.syncStamina(player);
                }
            })
            .withDrinkEffect((entity, magnitude) -> {
                if (entity instanceof net.minecraft.server.level.ServerPlayer player) {
                    int max = zcylas.totality.api.stamina.PlayerStaminaManager.getMaxStamina(player);
                    int amount = magnitude < 0 ? max : Math.round(magnitude);
                    zcylas.totality.api.stamina.PlayerStaminaManager.addStamina(player, amount);
                    zcylas.totality.networking.stamina.StaminaServerTick.syncStamina(player);
                }
            })
            .withDescription((magnitude, ticks) ->
    ...
    public static final AlchemyEffect FORTIFY_STAMINA = AlchemyEffect.register(
            ...
            .withConsumeEffect(entity -> {
                if (entity instanceof net.minecraft.world.entity.player.Player player) {
                    int currentMax = zcylas.totality.api.stamina.PlayerStaminaManager.getMaxStamina(player);
    ...
    public static final AlchemyEffect RESTORE_MANA = AlchemyEffect.register(
            ...
            .withConsumeEffect(entity -> {
                if (entity instanceof net.minecraft.server.level.ServerPlayer player) {
                    ...
                    zcylas.totality.api.mana.PlayerManaManager.addMana(player, 5);
                    zcylas.totality.networking.mana.ManaServerTick.syncMana(player);
                }
            })
            .withDrinkEffect((entity, magnitude) -> {
                if (entity instanceof net.minecraft.server.level.ServerPlayer player) {
                    int max = zcylas.totality.api.mana.PlayerManaManager.getMaxMana(player);
                    // magnitude < 0 = ULTIMATE — fully restore
                    int amount = magnitude < 0 ? max : Math.round(magnitude);
                    zcylas.totality.api.mana.PlayerManaManager.addMana(player, amount);
                    zcylas.totality.networking.mana.ManaServerTick.syncMana(player);
                }
            })
            .withDescription((magnitude, ticks) ->
    ...
    public static final AlchemyEffect FORTIFY_MANA = AlchemyEffect.register(
            ...
            .withConsumeEffect(entity -> {
                if (entity instanceof net.minecraft.world.entity.player.Player player) {
                    int currentMax = zcylas.totality.api.mana.PlayerManaManager.getMaxMana(player);
    ...
    public static final AlchemyEffect DAMAGE_STAMINA = AlchemyEffect.register(
            ...
            .withConsumeEffect(entity -> {
                if (entity instanceof net.minecraft.server.level.ServerPlayer player) {
                    zcylas.totality.api.stamina.PlayerStaminaManager.removeStamina(player, 5);
                    zcylas.totality.networking.stamina.StaminaServerTick.syncStamina(player);
                }
            })
            .withDrinkEffect((entity, magnitude) -> {
                if (entity instanceof net.minecraft.server.level.ServerPlayer player) {
                    zcylas.totality.api.stamina.PlayerStaminaManager.removeStamina(player, Math.round(magnitude));
                    zcylas.totality.networking.stamina.StaminaServerTick.syncStamina(player);
                }
            })
            .withDescription((magnitude, ticks) ->
