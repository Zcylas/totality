...
/**
 * A two-argument predicate with static {@link #and}, {@link #or}, and {@link #not} combinators.
 *
 * Ported from CreativeCore {@code BiFilter} (team.creative.creativecore).
 */
@FunctionalInterface
public interface BiFilter<T, U> {
    ...
    record BiFilterAnd<T, U>(BiFilter<T, U>[] filters) implements BiFilter<T, U> {
        @Override
        public boolean is(T t, U u) {
            for (BiFilter<T, U> f : filters)
                if (!f.is(t, u)) return false;
            return true;
        }
    }
