...
/**
 * Main TAB menu — uniform 3x3 grid.
 * Triangle points FROM center TOWARD selected panel (tip faces panel).
 * TODO: Remove /totality stats command — TAB menu is the entry point.
 */
public class MainMenuScreen extends Screen {
    ...
    private static final int CENTER_H = 36;
    // Fixed distance from center edge to triangle tip
    private static final int TRI_GAP  = 6;
    private static final int TRI_SIZE = 5;
    // Fixed distance from panel corner to diamond
    ...
    /**
     * Triangle tip points TOWARD the selected panel, drawn at center button edge.
     * For corners: diamond drawn at fixed offset from the inner corner of the panel.
     */
    private void drawTriangle(GuiGraphicsExtractor g, int ba) {
        ...
        if (isCorner) {
            // Diamond at fixed offset from the corner panel's inner corner
            int[] pb = drawnBounds(curRow, curCol);
            // Inner corner = corner closest to center
            int icx = (curCol == 0) ? pb[0] + pb[2] : pb[0]; // right edge if left col, left edge if right col
            int icy = (curRow == 0) ? pb[1] + pb[3] : pb[1]; // bottom edge if top row, top edge if bottom row
            // Place diamond at fixed offset inward from corner
    ...
    private void enterCurrent() {
        playClick();
        if (curRow == 1 && curCol == 1) { closeMenu(); return; }
        String label = LABELS[curRow][curCol];
    ...
    private void closeMenu()               { fadeOutTo(() -> Minecraft.getInstance().setScreen(null)); }
